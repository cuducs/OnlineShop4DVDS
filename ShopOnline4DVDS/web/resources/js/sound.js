...
var current_volume = 100; // ver 1.2c; 3/20/2013; Now current_volume is saved between songs
if (detect_browser() == "Mobile" || detect_browser() == "Chrome")
	current_volume = 1; // ver 1.4b - html5 audio volume is 1 = max .5 = half
...
+ '<param name="volume" value="'+current_volume+'">'            
...
else if (detect_browser() == "Mobile" || detect_browser() == "Chrome")
{
player.innerHTML = '<audio id="audio" autoplay preload="auto" onended="next_song(2);">'
+ '<source id="mp3" src="'+media+'" type="audio/mpeg" />'
+ '</audio>';
//document.getElementById('audio').load(); // ver 1.4b Needed for Chrome for Android
document.getElementById('audio').play(); // ver 1.4b Needed for Chrome for Android
}
...
function play_song()
{
	...
	if (document.getElementById('player').innerHTML == '')
	{
		...
		if (document.getElementById('audio'))	
			document.getElementById('audio').volume = current_volume;
	}
...
function stop_song()
{
	...
	if (document.getElementById('audio'))
		document.getElementById('audio').onended = null;
...
function detect_browser()
{
	...
	if (browser_name.indexOf("Opera")!= -1)
		browser_name = "Opera";
	else if (browser_name.match(/(iPad|iPhone|iPod|Android)/gi))
		browser_name = "Mobile";
...
{
	current_volume = Math.round(document.getElementById('audio').volume * 10)/10;
	...
	if (dir == 1 || dir == 5) // left or down
	{	
		if (current_volume > 0) // ver 1.4b
		document.getElementById('audio').volume = current_volume - 0.1;
		if (dir == 1)
			t = setTimeout('volume('+dir+')', 100);
	}
	else if (dir == 2) // right or up
	{
		if (current_volume < 1) // ver 1.4b
			document.getElementById('audio').volume = current_volume + 0.1;
		t = setTimeout('volume('+dir+')', 100);
	}
	else if (dir == 3) // stop changing volume
...
function display_time()
{
	...
	else if (detect_browser() == "Mobile" || detect_browser() == "Chrome")
	{
		...
		{	
			if (document.getElementById('songtime'))
				document.getElementById('songtime').innerHTML = 'Buffering: '+readyState+'0%';
			t2 = setTimeout('display_time();', 250); // recall this function every 250 ms
			return;
		}
		
		duration = Math.floor(document.getElementById('audio').duration);
		time = Math.floor(document.getElementById('audio').currentTime);
		/*document.getElementById('audio').addEventListener("ended", function end_song() 
		{
     		// song has ended
     		document.getElementById('audio').removeEventListener("ended", end_song, false);
	...
	if (detect_browser() != "MSIE" && detect_browser() != "Netscape")	// Firefox or Mobile
	{
...
function display_info()
{
	...
	else if (detect_browser() != "Mobile" && 
		detect_browser() != "Chrome") // Firefox Quicktime
	{
		// if quicktime GetVolume not supported
		if (typeof document.sound.GetVolume != 'function')
		{
...
function fast_forward()
{
	...
	{
		current_rate = document.getElementById('audio').playbackRate;	
		...
		document.getElementById('audio').pause();
		document.getElementById('audio').playbackRate = current_rate;
		document.getElementById('audio').play();
	}
